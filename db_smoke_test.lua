-- Live-DB smoke test for melodic.lua's Postgres write paths (mirrors
-- nexus/sapphire/symphony's db_smoke_test.lua pattern). Runs against the
-- real discord_music_melodic database (already-migrated, shared with the
...
-- touch real guild data (e.g. the pre-existing swarm_health/melodic_metrics
-- row for guild 1247394560007471134).
--
-- Run with: MELODIC_DRY_RUN=test MELODIC_DB_PORT=5432 luajit db_smoke_test.lua

package.path = "./lib/?.lua;./lib/?/init.lua;" .. package.path

-- Expects MELODIC_DRY_RUN=test in the environment already (set by the caller,
-- e.g. `MELODIC_DRY_RUN=test MELODIC_DB_PORT=5432 luajit db_smoke_test.lua`).
local A = dofile("melodic.lua")
...
A.q("UPDATE melodic_guild_settings SET volume = %s, loop_mode = %s WHERE guild_id = %s", 77, "song", TEST_GUILD)
...
local rows = A.q("SELECT video_url, title FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic' ORDER BY id ASC", TEST_GUILD)
...
local backup_count = A.q1("SELECT COUNT(*)::int AS n FROM melodic_queue_backup WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
...
A.q("DELETE FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
...
local backup_after = A.q1("SELECT COUNT(*)::int AS n FROM melodic_queue_backup WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
...
-- insert, per the live schema introspected via `\d melodic_track_intelligence`
-- and `\d melodic_user_track_affinity` -- both confirmed NOT NULL with no
...
local ti = A.q1("SELECT queued_count, play_count, finish_count, like_count, dislike_count, skip_count, total_listen_seconds FROM melodic_track_intelligence WHERE guild_id = %s AND url_key = %s", TEST_GUILD, key_a)
...
local aff = A.q1("SELECT queued_count, play_count, finish_count, skip_count, like_count, dislike_count, score FROM melodic_user_track_affinity WHERE guild_id = %s AND user_id = %s AND url_key = %s", TEST_GUILD, TEST_USER, key_a)
...
local ti2 = A.q1("SELECT queued_count FROM melodic_track_intelligence WHERE guild_id = %s AND url_key = %s", TEST_GUILD, A.track_key("https://example.com/a", "Track A"))
...
A.q([[INSERT INTO melodic_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, is_paused, title, play_session_key, track_uid)
      VALUES (%s, 'melodic', %s, %s, 42, true, false, %s, %s, %s)
...
local ps = A.q1("SELECT position_seconds, is_playing FROM melodic_playback_state WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
...
ps = A.q1("SELECT video_url, is_playing, position_seconds FROM melodic_playback_state WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
...
-- `\d melodic_voice_state`: reconnect_attempts is NOT NULL, no default)
...
local vs = A.q1("SELECT connected_channel_id::text AS connected_channel_id, desired_connected, reconnect_attempts FROM melodic_voice_state WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
...
vs = A.q1("SELECT connected_channel_id, desired_connected FROM melodic_voice_state WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
check("voice_state disconnected row correct", vs and vs.connected_channel_id == nil and vs.desired_connected == false, tostring(vs))

-- heartbeat_tick / melodic_metrics (the %s-placeholder-count vs args-supplied
...
local metrics = A.q1("SELECT queue_count, is_playing_db, is_paused_db, position_seconds, duration_seconds FROM melodic_metrics WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
check("melodic_metrics row written", metrics ~= nil, tostring(metrics))
check("melodic_metrics is_paused_db is boolean false, not a position/duration value", metrics and metrics.is_paused_db == false, metrics and tostring(metrics.is_paused_db))
check("melodic_metrics position_seconds is a plausible position (current_position, not duration/title)", metrics and tonumber(metrics.position_seconds) ~= nil and tonumber(metrics.position_seconds) < 300, metrics and tostring(metrics.position_seconds))
check("melodic_metrics duration_seconds == 200 (not swapped with position)", metrics and tonumber(metrics.duration_seconds) == 200, metrics and tostring(metrics.duration_seconds))
...
local pst = A.q1("SELECT position_seconds FROM melodic_playback_state WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
...
local ee = A.q1("SELECT title, error_type FROM melodic_error_events WHERE guild_id = %s AND error_type = 'smoke_test' ORDER BY created_at DESC LIMIT 1", TEST_GUILD)
...
A.q("DELETE FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
A.q("DELETE FROM melodic_queue_backup WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
A.q("DELETE FROM melodic_playback_state WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
A.q("DELETE FROM melodic_guild_settings WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM melodic_swarm_toggles WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM melodic_track_intelligence WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM melodic_user_track_affinity WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM melodic_voice_state WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)
A.q("DELETE FROM melodic_error_events WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM melodic_metrics WHERE guild_id = %s AND bot_name = 'melodic'", TEST_GUILD)

local leftover = A.q1([[
  SELECT
    (SELECT COUNT(*) FROM melodic_queue WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM melodic_queue_backup WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM melodic_playback_state WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM melodic_guild_settings WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM melodic_track_intelligence WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM melodic_user_track_affinity WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM melodic_voice_state WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM melodic_error_events WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM melodic_metrics WHERE guild_id = %s) AS n
...
local real_guild = A.q1("SELECT is_playing_db FROM melodic_metrics WHERE guild_id = %s AND bot_name = 'melodic'", "1247394560007471134")
check("pre-existing real guild_id 1247394560007471134 row in melodic_metrics untouched", real_guild ~= nil, tostring(real_guild))
