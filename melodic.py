...
async def delete_live_queue_copies(cur, guild_id, video_url, title):
    await cur.execute(
        "DELETE FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic' AND (video_url = %s OR title = %s)",
        (guild_id, video_url, title),
    )
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    ...
    title = _row_value(row, "title", _row_value(row, 1))
    await cur.execute(
        "SELECT video_url FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic' ORDER BY id ASC LIMIT 1",
        (guild_id,),
    )
    existing = await cur.fetchone()
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "SELECT position_seconds FROM melodic_playback_state WHERE guild_id = %s AND bot_name = 'melodic' AND is_playing = TRUE LIMIT 1",
                        (target_guild_id,),
                    )
...
async def on_wavelink_node_ready(payload: wavelink.NodeReadyEventPayload):
    ...
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT guild_id, channel_id, position_seconds, video_url, title FROM melodic_playback_state WHERE is_playing = TRUE AND bot_name = 'melodic'")
                    ...
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            if queue_rows:
                                await cur.executemany(
                                    "INSERT INTO melodic_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)",
                                    [(guild_id, 'melodic', t_url, t_title, req_id) for t_url, t_title, req_id in queue_rows],
                                )
                                try:
