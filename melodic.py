...
async def build_autodj_query(cur, guild_id):
    await cur.execute("SELECT title FROM melodic_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 8", (guild_id,))
...
async def maybe_enqueue_autodj(cur, guild, channel_id):
    ...
    try:
        ...
        history_titles = set()
        await cur.execute("SELECT title FROM melodic_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 12", (guild.id,))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS melodic_swarm_direct_orders (id INT AUTO_INCREMENT PRIMARY KEY, bot_name VARCHAR(50), guild_id BIGINT, vc_id BIGINT, text_channel_id BIGINT, command VARCHAR(50), data TEXT, attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL)")
            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    for g in guilds:
                        ...
                        await cur.execute("SELECT COUNT(*) AS total FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic'", (g.id,))
                        queue_row = await cur.fetchone() or {}
                        await cur.execute("SELECT COUNT(*) AS total FROM melodic_queue_backup WHERE guild_id = %s AND bot_name = 'melodic'", (g.id,))
                        backup_row = await cur.fetchone() or {}
                        await cur.execute("SELECT is_playing, is_paused, position_seconds FROM melodic_playback_state WHERE guild_id = %s AND bot_name = 'melodic' LIMIT 1", (g.id,))
...
async def ensure_swarm_command_tables():
    try:
        ...
        logger.exception("Failed ensuring swarm command tables for melodic.")
...
async def direct_order_listener():
    try:
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT id, bot_name, guild_id, vc_id, text_channel_id, command, data, COALESCE(attempts, 0) AS attempts FROM melodic_swarm_direct_orders WHERE bot_name = %s ORDER BY id ASC", ('melodic',))
                ...
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        if executed or attempts >= 2 or not guild:
                            ...
                            await cur.execute("UPDATE melodic_swarm_direct_orders SET attempts = COALESCE(attempts, 0) + 1, last_error = %s WHERE id = %s", (f"unexecuted:{cmd}", oid))
