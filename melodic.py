    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT id, video_url, title FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic' ORDER BY id ASC LIMIT %s", (interaction.guild.id, index-1))
                rows = await cur.fetchall()
                for r in rows:
                    await cur.execute("DELETE FROM melodic_queue WHERE id = %s AND guild_id = %s AND bot_name = 'melodic'", (r[0], interaction.guild.id))
                    await cur.execute(
                        "DELETE FROM melodic_queue_backup WHERE video_url = %s AND title = %s AND guild_id = %s AND bot_name = 'melodic' LIMIT 1",
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                for url, title in q:
                    await cur.execute("INSERT INTO melodic_user_playlists (user_id, playlist_name, video_url, title) VALUES (%s, %s, %s, %s)", (interaction.user.id, name, url, title))
        ...
        try:
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        await cur.execute("CREATE TABLE IF NOT EXISTS melodic_swarm_overrides (guild_id BIGINT, bot_name VARCHAR(50), command VARCHAR(20), attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL, PRIMARY KEY(guild_id, bot_name))")
                        await cur.execute("CREATE TABLE IF NOT EXISTS melodic_swarm_direct_orders (id INT AUTO_INCREMENT PRIMARY KEY, bot_name VARCHAR(50), guild_id BIGINT, vc_id BIGINT NULL, text_channel_id BIGINT NULL, command VARCHAR(50), data TEXT NULL, attempts INT NOT NULL DEFAULT 0, last_error TEXT NULL)")
                        for stmt in [
