    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if not next_song:
                    await cur.execute("UPDATE melodic_playback_state SET is_playing = FALSE, is_paused = FALSE WHERE guild_id = %s AND bot_name = 'melodic'", (guild.id,))
...
async def clear(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic'", (interaction.guild.id,))
                await cur.execute("UPDATE melodic_playback_state SET is_playing = FALSE, is_paused = FALSE, position_seconds = 0 WHERE guild_id = %s AND bot_name = 'melodic'", (interaction.guild.id,))
    await bot.change_presence(status=discord.Status.online)
...
async def remove(interaction: discord.Interaction, index: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic' ORDER BY id ASC LIMIT 1 OFFSET %s", (interaction.guild.id, index-1))
                row = await cur.fetchone()
                if row:
...
async def skipto(interaction: discord.Interaction, index: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic' ORDER BY id ASC LIMIT %s", (interaction.guild.id, index-1))
                rows = await cur.fetchall()
                for r in rows: await cur.execute("DELETE FROM melodic_queue WHERE id = %s AND guild_id = %s AND bot_name = 'melodic'", (r[0], interaction.guild.id))
            ...
            if guild:
                ...
                if cmd == 'PLAY':
                    ...
                    if recover_channel_id:
                        ...
                        logger.warning("[melodic] Could not resolve RECOVER target for guild %s.", guild.id)
