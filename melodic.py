...
DB_CONFIG = {
    ...
    'db': os.getenv(f"{BOT_ENV_PREFIX}_DB_NAME", "discord_music_melodic"),
    'autocommit': True
}
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("CREATE TABLE IF NOT EXISTS melodic_playback_state (guild_id BIGINT, bot_name VARCHAR(50), channel_id BIGINT, video_url TEXT, position_seconds INT DEFAULT 0, is_playing BOOLEAN DEFAULT FALSE, title TEXT, PRIMARY KEY (guild_id, bot_name))")
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS melodic_swarm_direct_orders (id INT AUTO_INCREMENT PRIMARY KEY, bot_name VARCHAR(50), guild_id BIGINT, vc_id BIGINT, text_channel_id BIGINT, command VARCHAR(50), data TEXT)")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                            ...
                            async with dj_pool.acquire() as dj_conn:
                                async with dj_conn.cursor(aiomysql.DictCursor) as dj_cur:
                                    ...
                                    if dj_res and dj_res.get('auto_dj'):
                                        ...
                                        await dj_cur.execute("INSERT INTO melodic_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'melodic', %s, %s, %s)", (guild.id, f"ytsearch:{genre} track", f"📻 Auto-DJ: {genre}", bot.user.id))
                ...
                await cur.execute("DELETE FROM melodic_queue WHERE id = %s", (song_id,))
            ...
            async with conn.cursor() as cur:
                for guild_id, data in list(playback_tracking.items()):
                    ...
                    if guild and guild.voice_client and getattr(guild.voice_client, 'playing', False):
                        ...
                        await cur.execute("REPLACE INTO melodic_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, title) VALUES (%s, 'melodic', %s, %s, %s, TRUE, %s)", (guild_id, data['channel_id'], data['url'], pos, data['title']))
                        last_position_persist[guild_id] = now

# --- SETTINGS COMMANDS ---
@bot.tree.command(name="melodic_main_sethome", description="Set bot's default voice/stage channel")
...
@bot.tree.command(name="melodic_main_restart", description="Forcefully restart the bot instance (Admins only)")
@commands.has_permissions(administrator=True)
...
# --- PLAYBACK COMMANDS ---
@bot.tree.command(name="melodic_main_play", description="Play a song, link, livestream, or YouTube playlist")
...
@bot.tree.command(name="melodic_main_playnext", description="Put song at top of queue")
...
async def clear(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic'", (interaction.guild.id,))
                await cur.execute("UPDATE melodic_playback_state SET is_playing = FALSE, position_seconds = 0 WHERE guild_id = %s AND bot_name = 'melodic'", (interaction.guild.id,))
    await bot.change_presence(status=discord.Status.online)
...
@bot.tree.command(name="melodic_main_queue", description="View queue")
async def queue_cmd(interaction: discord.Interaction):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT title FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic' ORDER BY id ASC LIMIT 10", (interaction.guild.id,))
...
@bot.tree.command(name="melodic_main_shuffle", description="Shuffle the queue")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("DELETE FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic'", (interaction.guild.id,))
                for row in l: await cur.execute("INSERT INTO melodic_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'melodic', %s, %s, %s)", (interaction.guild.id, row[1], row[2], row[3]))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if row: 
                    await cur.execute("DELETE FROM melodic_queue WHERE id = %s", (row[0],))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                for r in rows: await cur.execute("DELETE FROM melodic_queue WHERE id = %s", (r[0],))
...
@bot.tree.command(name="melodic_main_move", description="Move song to new position")
...
# --- PLAYLISTS & HISTORY ---
@bot.tree.command(name="melodic_main_savequeue", description="Save the current queue as a custom personal playlist")
...
@bot.tree.command(name="melodic_main_loadqueue", description="Load a custom personal playlist into the current queue")
...
@bot.tree.command(name="melodic_main_leaderboard", description="Show the top 10 most played tracks in this server")
...
@bot.tree.command(name="melodic_main_history", description="Show last 5 songs played in the server")
...
@bot.tree.command(name="melodic_main_userhistory", description="See the last 10 tracks requested by a specific user")
...
@bot.tree.command(name="melodic_main_grab", description="DM yourself the current song")
...
# --- MODIFIERS & FILTERS ---
@bot.tree.command(name="melodic_main_volume", description="Set volume (1-200)")
...
@bot.tree.command(name="melodic_main_filter", description="Apply an audio filter to the music")
...
@bot.tree.command(name="melodic_main_modify", description="Change speed and pitch for upcoming tracks")
...
# --- SCRUBBING ---
@bot.tree.command(name="melodic_main_seek", description="Seek to seconds")
...
@bot.tree.command(name="melodic_main_forward", description="Skip forward X seconds")
...
@bot.tree.command(name="melodic_main_rewind", description="Rewind X seconds")
...
@bot.tree.command(name="melodic_main_replay", description="Restart current song")
...
@bot.tree.command(name="melodic_main_stats", description="Bot statistics")
...
async def aria_command_listener():
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT guild_id, command FROM melodic_swarm_overrides WHERE bot_name = %s", ('melodic',))
...
async def direct_order_listener():
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT * FROM melodic_swarm_direct_orders WHERE bot_name = %s", ('melodic',))
                        ...
                        if _has_human_listeners(guild.voice_client) and not force_leave:
                            ...
                            async with DBPoolManager() as pool:
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        await cur.execute("DELETE FROM melodic_queue WHERE guild_id = %s AND bot_name = 'melodic'", (guild.id,))
                                        await cur.execute("REPLACE INTO melodic_playback_state (guild_id, bot_name, is_playing) VALUES (%s, 'melodic', FALSE)", (guild.id,))
                ...
                if guild.voice_client and getattr(guild.voice_client, 'playing', False):
                    ...
                    if not track_title:
                        async with DBPoolManager() as pool:
                            async with pool.acquire() as conn:
                                async with conn.cursor(aiomysql.DictCursor) as cur:
                                    await cur.execute("SELECT title FROM melodic_playback_state WHERE guild_id = %s AND bot_name = 'melodic' AND is_playing = TRUE LIMIT 1", (guild.id,))
                                    res = await cur.fetchone()
