...
![Melodic bot icon](assets/bot-icon.png)

Melodic is one node in the 13-bot Discord music fleet. Focused on smooth, melody-forward queues. It owns its queue tables, reports playback state to the shared swarm database, and is controlled directly from Discord, Aria, and SwarmPanel.

## Identity

- Discord display name: **GWS Music Bot (Melodic)**
- Internal node name: `melodic`
- Command namespace: `/melodic_main_*`
...
All commands are namespaced as `/melodic_main_<command>`.
...
- Node: `melodic`
- Database schema: `discord_music_melodic`
